...
class TTPixelImage:
    ...
    @staticmethod
    def _resize_image(image, width_pixel: float, height_pixel: float):
    ...
    @staticmethod
    def _setup_turtle(width, height, pixel_size):
    ...
    @staticmethod
    def _draw_image(im, width, height, pixel_size):
    ...
    def tt_draw_picture(url: str, pixel_size: int = 5, width_pixel: float = 1.0, height_pixel: float = 1.0):
        ...
        url = TTPixelImage.fetch_image(url)
        if not url:
            return

        with Image.open(url) as im:
            im = TTPixelImage._resize_image(im, width_pixel, height_pixel)
            im = np.array(im) / 255.0
            width, height = im.shape[1], im.shape[0]

            TTPixelImage._setup_turtle(width, height, pixel_size)
            TTPixelImage._draw_image(im, width, height, pixel_size)
